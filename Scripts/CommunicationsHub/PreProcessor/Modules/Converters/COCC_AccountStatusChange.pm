package Converters::COCC_AccountStatusChange;
...
# Module Globals
#
my $DOCUMENT_TYPE   = q(AccountStatusChange);    # used for output JSON file postfix
my $FILENAME_SUFFIX = q(CO_STCHG_NOTICE);        # used for input XML file postfix
...
sub handle_document_details {
	...
	my $doc_details = setCOCCDocumentDetails(
		$set,
		{
			'ACCOUNT_STATUS'       => 'Account_Information/Acct_Status_Desc',
			'PRIOR_ACCOUNT_STATUS' => 'Account_Information/Prior_Acct_Status',
	...
	$doc_details->{'TEMPLATE'}      = 'Account Status Change Notice';
